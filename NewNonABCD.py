...
def circle(r):
    ...
    if forward[-1][0] < input.droplet_pos[0]:
        raise ValueError
    else:
        forward[-1][1] = np.tan(forward[-1][2]) * (input.droplet_pos[0] - forward[-1][0]) + forward[-1][1]
        forward[-1][0] = input.droplet_pos[0]
        backward[-1][1] = np.tan(forward[-1][2]) * (input.droplet_pos[0] - forward[-1][0]) + forward[-1][1]
        backward[-1][0] = input.droplet_pos[0]

    if forward[-1][1] >= r + input.droplet_pos[1] or forward[-1][1] <= input.droplet_pos[1] - r:
        propagate(r)

    else:
        circle_center = (input.droplet_pos[0], input.droplet_pos[1])
        ...
        # # snell's law
        tilt_1 = np.arcsin((forward[-1][1]-input.droplet_pos[1])/ r)
        flat_interface(-tilt_1, input.medium_n, input.target_n)
        state = forward[-1]
        a = input.droplet_pos[0]
        b = input.droplet_pos[1]
        k = rad2slope(state[2])
        c = state[1] - k*state[0]
        ...
        tilt_2 = np.arcsin((forward[-1][1]-input.droplet_pos[1]) / r)
...
if __name__ == '__main__':
    ...
    def bundle():
        ...
        for i in range(input.no_of_rays):
            ray(0, list_of_rays[i], 0, intensity(list_of_rays)[i])
            lens_trace(input.lens_f, input.lens_pos, input.len_thickness)
            free_propagate(10)
            propagate(input.droplet_pos[0]-input.lens_pos-10)
            circle(input.radius)
            free_propagate(15)
            propagate(200)
            a = copy.deepcopy(forward)
            b = copy.deepcopy(backward)
            forward.clear()
            backward.clear()
    ...
    def plot():
        ...
        for i in range(input.no_of_rays):
            ...
            x_c = input.radius * np.cos(theta) + input.droplet_pos[0]
            y_c = input.radius * np.sin(theta) + input.droplet_pos[1]
            plt.plot(x_c, y_c)
    ...
    def plot_detector(positions):
        plt.figure('cross_section intensity')
        for i in range(len(positions)):
            y_values = detector(positions[i])[0]
            intns_values = detector(positions[i])[1]
    ...
    plot_detector([100,200,300,350,400])
